...
def generate_fig(data,logP_slider,logFC_pos_slider,logFC_neg_slider):
    fig = px.scatter(data, x="-logp", y="logFC", size="dot_size",
    ...
    df_anno = data[(data['-logp'] >= logP_slider) & (
                (data['logFC'] >= logFC_pos_slider) | (data['logFC'] <= logFC_neg_slider))]
    annotations = [{
        'x': row['-logp'],
        'y': row['logFC'],
        'xref': 'x',
...
def update_graph(datafile, logFC_slider_value, logP_slider, selectedData):
    if datafile is None:
        return go.Figure()
    ...
    df_data_hits.to_csv('test/data_hits.csv')
    ...
    if selectedData:
        ...
        df_click = df_data_hits.copy(deep=True)
        df_click['opacity'] = 0.25
        for i, row in df_click.iterrows():
            if row['Accession'] in connected_uniprots:
                df_click.at[i, 'color'] = 'green'
                df_click.at[i, 'opacity'] = 1
        df_click.to_csv('test/df_click.csv')
        fig = px.scatter(df_click, x="-logp", y="logFC", size="dot_size",
                        color=df_click['color'], opacity=df_click['opacity'],
                        hover_data=["freq_hit", "Gene.Symbol", "Accession"])
        fig['data'][0]['marker']['color'] = df_click['color']
        fig['data'][0]['marker']['opacity'] = df_click['opacity']
        df_anno = df_click[(df_click['-logp'] >= logP_slider) & (
                    (df_click['logFC'] >= logFC_pos_slider) | (df_click['logFC'] <= logFC_neg_slider))]
        
        # fig = generate_fig(df_click, logP_slider, logFC_pos_slider, logFC_neg_slider)
    else:
        df_data_hits['color'] = np.select(conditions, ['red', 'blue'], 'gray')
        df_data_hits['opacity'] = np.select(conditions, [1, 1], 0.25)
        df_data_hits.to_csv('test/df_data_hits2.csv')
        fig = px.scatter(df_data_hits, x="-logp", y="logFC", size="dot_size",
                        hover_data=["freq_hit", "Gene.Symbol", "Accession"])
        fig['data'][0]['marker']['color'] = df_data_hits['color']
        fig['data'][0]['marker']['opacity'] = df_data_hits['opacity']
        df_anno = df_data_hits[(df_data_hits['-logp'] >= logP_slider) & (
                    (df_data_hits['logFC'] >= logFC_pos_slider) | (df_data_hits['logFC'] <= logFC_neg_slider))]
        
        # fig = generate_fig(df_data_hits, logP_slider, logFC_pos_slider, logFC_neg_slider)
        '''
