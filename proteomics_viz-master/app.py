...
def update_graph(datafile, logFC_slider_value, logP_slider, selected_data):
    ...
    if selected_data:
        ...
        df_data_hits['color'] = np.where(df_data_hits['Accession'].isin(connected_uniprots), 'green',
                                         df_data_hits['color'])
        df_data_hits['opacity'] = np.where(df_data_hits['Accession'].isin(connected_uniprots), 1, 0.25)
        print(df_data_hits['color'],df_data_hits['opacity'])
        # for i, row in df_data_hits.iterrows():
        #     if row['Accession'] in connected_uniprots:
        #         df_data_hits.loc[i, 'color'] = 'green'
        #         df_data_hits.loc[i, 'opacity'] = 1
        #     else:
        #         df_data_hits.loc[i, 'opacity'] = 0.75
        # opacity = [2.0 if acc in connected_uniprots else 0.2 for acc in df_data_hits['Accession']]
        # color = ['green' if acc in connected_uniprots else color for acc, color in
        #          zip(df_data_hits['Accession'], fig['data'][0]['marker']['color'])]

        fig.update_traces(
            marker=dict(color=df_data_hits['color'], opacity=df_data_hits['opacity'])
            # selected=dict(marker=dict(color=df_data_hits['color'], opacity=df_data_hits['opacity'])),
            # unselected=dict(marker=dict(color=df_data_hits['color'], opacity=df_data_hits['opacity']))
        )
    else:
        df_data_hits['color'] = np.select(conditions, ['red', 'blue'], 'gray')
        df_data_hits['opacity'] = np.select(conditions, [1, 1], 0.25)
